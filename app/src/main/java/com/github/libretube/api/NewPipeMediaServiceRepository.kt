...
class NewPipeMediaServiceRepository : MediaServiceRepository {
    ...
    override suspend fun getStreams(videoId: String): Streams = withContext(Dispatchers.IO) {
        ...
        val loungeMetadataAsync = async {
            runCatching { fetchLoungeMetadata(videoId) }.getOrNull()
        }
        ...
        val loungeMetadata = loungeMetadataAsync.await()
        ...
        Streams(
            ...
            listId = loungeMetadata?.listId,
            params = loungeMetadata?.params,
            playerParams = loungeMetadata?.playerParams
        )
    }

    private suspend fun fetchLoungeMetadata(videoId: String): LoungeMetadata {
        ...
        val nextResponse = getJsonPostResponse("next", body, Localization.DEFAULT)
        return LoungeMetadata(
            listId = findFirstString(nextResponse, "playlistId"),
            params = findFirstString(nextResponse, "params"),
            playerParams = findFirstString(nextResponse, "playerParams"),
        )
    }

    private fun findFirstString(json: JsonObject, targetKey: String): String? {
            ...
            when (val element = stack.removeLast()) {
                is JsonObject -> {
                    for ((key, value) in element) {
                        if (key == targetKey && value is String) return value
                        if (value is JsonObject || value is JsonArray) stack.add(value)
                    }
                }

                is JsonArray -> {
                    element.forEach { item ->
                        if (item is JsonObject || item is JsonArray) stack.add(item)
                    }
                }
            }
        }
    ...
    private data class LoungeMetadata(
        val listId: String?,
        val params: String?,
